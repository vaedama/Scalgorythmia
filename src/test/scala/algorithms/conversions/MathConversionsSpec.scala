...
class MathConversionsSpec extends FunSuite {
  ...
  test("getNthDigit") {
    assert(MathConversions.getNthDigit(1024, 10, 0) === BigInt(4))
    assert(MathConversions.getNthDigit(1024, 10, 1) === BigInt(2))
    assert(MathConversions.getNthDigit(1024, 10, 2) === BigInt(0))
    assert(MathConversions.getNthDigit(1024, 10, 3) === BigInt(1))
  }
