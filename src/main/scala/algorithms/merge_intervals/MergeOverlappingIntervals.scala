...
// Intuition: When does an overlap occur?
// Given two intervals (prevFrom, prevTo) and (nextFrom, nextTo)
// an overlap occurs if nextFrom <= prevTo

// There are two possible overlapping scenarios:

// 1. Full overlap: nextFrom <= prevTo and nextTo <= prevTo
// Merged = (prevFrom, prevTo)
// [(1,7), (4,6)] => (1,7)

// 2. Partial overlap: nextTo > prevTo
// Merged = (prevFrom, nextTo)
// [(1,5), (3,7)] => (1,7)
object MergeOverlappingIntervals {
  ...
  private def isOverlap(prevTo: Int, nextFrom: Int): Boolean =
    nextFrom <= prevTo

  private def isFullOverlap(prevTo: Int, nextTo: Int): Boolean =
    prevTo >= nextTo
